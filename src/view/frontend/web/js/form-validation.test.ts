...
// The field macro renders the error node up front so it carries the styles and
// the aria-describedby target; the enhancer has to write into that one.
describe("enhanceValidation and the field macro's error node", () => {
    const buildMacroForm = (): HTMLFormElement => {
        document.body.innerHTML = `
            <form>
                <div class="field">
                    <label class="field__label" for="field-email">Email<span class="field__required" aria-hidden="true">*</span></label>
                    <input class="field__control" type="email" id="field-email" name="email" required aria-required="true" aria-describedby="field-email-error">
                    <p class="field__error" id="field-email-error" role="alert"></p>
                </div>
            </form>`;
        return document.querySelector("form") as HTMLFormElement;
    };

    it("fills the node that is already there instead of injecting a second one", () => {
        const form = buildMacroForm();
        enhanceValidation(form, { email: [required("Required.")] });

        form.dispatchEvent(new Event("submit", { cancelable: true }));

        expect(form.querySelectorAll(".field__error")).toHaveLength(1);
        const node = document.getElementById("field-email-error") as HTMLElement;
        expect(node.textContent).toBe("Required.");
        expect(node.classList.contains("field__error")).toBe(true);
        expect(document.getElementById("field-email")?.getAttribute("aria-invalid")).toBe("true");
    });

    it("empties the node rather than removing it once the value is fixed", () => {
        const form = buildMacroForm();
        enhanceValidation(form, { email: [required("Required.")] });
        form.dispatchEvent(new Event("submit", { cancelable: true }));

        const input = document.getElementById("field-email") as HTMLInputElement;
        input.value = "ada@shop.test";
    ...
    it("still works on a form that has no pre-rendered node", () => {
        document.body.innerHTML = `<form><input id="bare" name="email"></form>`;
        const form = document.querySelector("form") as HTMLFormElement;
        enhanceValidation(form, { email: [required("Required.")] });

        form.dispatchEvent(new Event("submit", { cancelable: true }));

        const node = document.getElementById("bare-error") as HTMLElement;
        expect(node?.className).toBe("field__error");
        expect(node?.textContent).toBe("Required.");
    });
});
